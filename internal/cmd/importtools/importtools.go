// SPDX-FileCopyrightText: Copyright 2021 The goimportz Authors
...
func init() {
	imports.LocalPrefix = "github.com/zchee/goimportz"
}

func main() {
	ctx := context.Background()
	pkgs, err := listPackages(ctx, nil,
	...
	for _, pkg := range pkgs {
		...
		default:
			...
			dir := filepath.Join(append([]string{"./"}, parts[3:]...)...)
...
func writeFile(path, body string) {
	...
	data, err := imports.Process(path, []byte(body), &imports.Options{
		TabWidth:  8,
		TabIndent: true,
		Comments:  true,
	})
...
func copyGoimports(pkg *Package) {
	for _, path := range sourceFiles(pkg) {
		...
		body = strings.ReplaceAll(body, "goimports", "goimportz")
		body = strings.ReplaceAll(body, "goimport's", "goimport'z")
		name = strings.ReplaceAll(name, "goimports", "goimportz")

		writeFile(filepath.Join("cmd", "goimportz", name), body)
	}
}
...
func copyInternal(pkg *Package, dir string) {
	for _, path := range sourceFiles(pkg) {
		...
		body = strings.ReplaceAll(body, "goimports", "goimportz")
		body = strings.ReplaceAll(body, "goimport's", "goimport'z")
		body = strings.ReplaceAll(body, "goimportzignore", "goimportsignore")
		name := filepath.Base(path)
...
func fixImports(body string) string {
	return strings.ReplaceAll(
		...
		"github.com/zchee/goimportz/internal/",
	)
}
