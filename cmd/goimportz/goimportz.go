...
import (
	...
	"github.com/zchee/goimportz/internal/gocommand"
	"github.com/zchee/goimportz/internal/imports"
)

var (
	// main operation modes
	list   = flag.Bool("l", false, "list files whose formatting differs from goimport'z")
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: goimportz [flags] [path ...]\n")
	flag.PrintDefaults()
	os.Exit(2)
}
...
// argumentType is which mode goimportz was invoked as.
type argumentType int

const (
	// fromStdin means the user is piping their source into goimportz.
	fromStdin argumentType = iota

	// singleArg is the common case from editors, when goimportz is run on
	// a single file.
	singleArg

	// multipleArg is when the user ran "goimportz file1.go file2.go"
	// or ran goimportz on a directory tree.
	multipleArg
)
...
func processFile(filename string, in io.Reader, out io.Writer, argType argumentType) error {
	...
	if *srcdir != "" {
		...
		if isFile(*srcdir) {
			...
			// The goimportz on-save hook writes the buffer to a temp file
			// first and runs goimportz before the actual save to newfile.go.
			// The editor's buffer is named "newfile.go" so that is passed to goimportz as:
			//      goimportz -srcdir=/gopath/src/pkg/newfile.go /tmp/gofmtXXXXXXXX.go
...
func gofmtMain() {
	...
	// used to allow goimportz to compile under gccgo, which does not support
